...
<form method="POST" action="{{ route('clusters.store') }}" class="space-y-8">
@csrf
...
class="border rounded-lg p-2 w-full mt-1">{{ old('description') }}</textarea>
</div>
...
<textarea name="ticker_text"
rows="3"
class="border rounded-lg p-2 w-full"
placeholder="Sale Today | Offers | Announcement">{{ old('ticker_text') }}</textarea>
</div>
...
@if(collect(old('screens'))->contains($screen->id)) selected @endif>
{{ $screen->name }}
</option>
@endforeach
